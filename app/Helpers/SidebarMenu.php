...
class SidebarMenu
{
    ...
    public static function getIcon(string $iconName): string
    {
        $icons = [
            ...
            'chat-bubble-left-right' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H4a2 2 0 00-2 2v7a2 2 0 002 2h2v4l4-4h4m-4-5h.01M14 13h.01M9 13h.01"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h3l3 3v-3a2 2 0 002-2v-5a2 2 0 00-2-2"/>',
    ...
    public static function isRouteActive(string $route, ?array $params = null): bool
    {
        ...
        if ($route === 'admin.approvals.index') {
            $approvalRoutes = [
                ...
                'admin.activities.*',
                'admin.endorsements.*',
                'admin.calibre-requests.*',
            ];
            foreach ($approvalRoutes as $approvalRoute) {
                if (request()->routeIs($approvalRoute)) {
                    return true;
                }
            }
        }
