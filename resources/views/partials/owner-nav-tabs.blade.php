    ...
    <div class="max-w-7xl mx-auto">
        <nav class="flex flex-wrap gap-x-1 -mb-px overflow-x-auto scrollbar-none">
            <a href="{{ route('owner.dashboard') }}" wire:navigate
               class="flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium border-b-2 {{ $currentRoute === 'owner.dashboard' ? $active : $inactive }} transition-colors whitespace-nowrap">
            ...
            <a href="{{ route('owner.admins.index') }}" wire:navigate
               class="flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium border-b-2 {{ $currentRoute === 'owner.admins.index' ? $active : $inactive }} transition-colors whitespace-nowrap">
                <svg class="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('owner.settings.index') }}" wire:navigate
               class="flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium border-b-2 {{ str_starts_with($currentRoute, 'owner.settings.') ? $active : $inactive }} transition-colors whitespace-nowrap">
                <svg class="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
