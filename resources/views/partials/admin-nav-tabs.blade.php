    ...
    <div class="max-w-7xl mx-auto">
        <nav class="flex flex-wrap gap-x-0.5 -mb-px overflow-x-auto scrollbar-none">
            ...
            @if(auth()->user()?->isSuperAdmin())
                @php
                    $pendingDeletions = \App\Models\UserDeletionRequest::pending()->count();
                @endphp
                <a href="{{ route('owner.deletion-requests.index') }}" wire:navigate
                   class="flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium border-b-2 {{ $currentRoute === 'owner.deletion-requests.index' ? $active : $inactive }} transition-colors whitespace-nowrap">
                    <svg class="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/>
                    </svg>
                    Deletions
                    @if($pendingDeletions > 0)
                        <span class="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-xs font-semibold bg-red-500 text-white">{{ $pendingDeletions }}</span>
                    @endif
                </a>
            @endif
