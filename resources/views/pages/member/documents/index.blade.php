...
new #[Layout('layouts.app.sidebar')] class extends Component {
    ...
    public function with(): array
    {
        ...
        $documents = MemberDocument::where('user_id', $user->id)
    ...
    public function deleteDocument(MemberDocument $document): void
    {
        ...
        // Close modal if viewing this document
        if ($this->viewingDocument && $this->viewingDocument->uuid === $document->uuid) {
...
<div>
            ...
            <div class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @foreach($documents as $doc)
                    <a href="{{ route('documents.show', $doc) }}" wire:navigate
                        class="block p-4 hover:bg-zinc-50 dark:hover:bg-zinc-700/30 transition-colors cursor-pointer">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="p-3 bg-zinc-100 dark:bg-zinc-700 rounded-lg">
                                    ...
                                    @else
                                        <svg class="w-6 h-6 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clip-rule="evenodd"/></svg>
                                    @endif
                                </div>
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $this->getStatusBadgeClass($doc->status) }}">
                                        ...
                                        @if($doc->file_purged_at)
                                            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium bg-zinc-100 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400">
                                                <svg class="size-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg>
                                                File removed
                                            </span>
                                        @endif
                                    </div>
                                    <p class="text-sm text-zinc-500 dark:text-zinc-400 truncate max-w-md">
    ...
    @else
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 p-12 text-center">
            ...
            <h3 class="mt-4 text-lg font-medium text-zinc-900 dark:text-white">No Documents</h3>
                        ...
                        @if($this->requiresIdMetadata())
                            <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800 space-y-4">
                                ...
                                <div class="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                                    <svg class="size-4 text-amber-600 dark:text-amber-400 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/>
                                    </svg>
                                    <p class="text-xs text-amber-700 dark:text-amber-300">
                                        <strong>POPIA Notice:</strong> Your ID document file will be automatically deleted from our servers 7 days after it has been approved. Only the verified metadata (name, ID number, date of birth) will be retained for your membership records.
                                    </p>
                                </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $this->getStatusBadgeClass($viewingDocument->status) }}">
                                    @if($viewingDocument->status === 'pending')
                                        Awaiting Approval
                                    @else
