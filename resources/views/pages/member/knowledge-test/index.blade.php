...
<div>
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
        ...
        <div class="grid gap-3 {{ count($this->dedicatedStatusRequirements) === 1 ? 'sm:grid-cols-1 max-w-md' : (count($this->dedicatedStatusRequirements) === 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-3') }}">
            ...
            <div class="rounded-lg border p-4 {{ $req['passed'] ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-900/20' : 'border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900' }}">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    @endif
                    <span class="font-medium {{ $req['passed'] ? 'text-emerald-800 dark:text-emerald-200' : 'text-zinc-900 dark:text-white' }}">{{ $req['label'] }}</span>
                </div>
                <p class="text-xs {{ $req['passed'] ? 'text-emerald-700 dark:text-emerald-300' : 'text-zinc-500 dark:text-zinc-400' }}">{{ $req['description'] }}</p>
                @if($req['passed'])
                <span class="mt-2 inline-block text-xs font-medium text-emerald-600 dark:text-emerald-400">✓ Qualified</span>
                @endif
            </div>
            @endforeach
        </div>
    ...
    <div class="rounded-xl border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
        <div class="flex items-center justify-between gap-4">
            ...
            <a href="{{ route('knowledge-test.take', $this->inProgressAttempt->knowledgeTest) }}" wire:navigate class="inline-flex items-center gap-2 rounded-lg bg-yellow-600 px-4 py-2 text-sm font-medium text-white hover:bg-yellow-700 transition-colors">
                Continue
    ...
    <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800">
            ...
            <div class="border-t border-zinc-200 bg-zinc-50 px-6 py-4 dark:border-zinc-700 dark:bg-zinc-900/50">
                ...
                <a href="{{ route('knowledge-test.take', $test) }}" wire:navigate class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-yellow-600 px-4 py-2 text-sm font-medium text-white hover:bg-yellow-700 transition-colors">
                    Continue Test
                    <svg class="size-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                    </svg>
                ...
                @else
                    @if($this->canApplyForDedicatedStatus)
                    <button wire:click="startTest({{ $test->id }})" wire:confirm="Are you sure you want to start this test? {{ $test->time_limit_minutes ? 'You will have ' . $test->time_limit_minutes . ' minutes to complete it.' : '' }}" class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-nrapa-blue px-4 py-2 text-sm font-medium text-white hover:bg-nrapa-blue-dark transition-colors">
                        Start Test
                        <svg class="size-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                        </svg>
