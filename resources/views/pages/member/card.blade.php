...
<div class="min-h-screen bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 flex flex-col">
    ...
    <div class="text-white px-4 py-3 text-center text-sm" style="background: #0B4EA2;">
        <div class="flex items-center justify-center gap-2">
            ...
            <button wire:click="dismissInstructions" class="ml-2 p-1 rounded" style="background: rgba(255,255,255,0.15);">
    ...
    <div class="flex-1 flex items-center justify-center p-4">
        ...
        <div class="w-full max-w-sm">
            {{-- Card Container --}}
            <div class="rounded-3xl shadow-2xl overflow-hidden" style="border: 1px solid rgba(11,78,162,0.3);">
                {{-- Card Header — NRAPA Blue with orange accent --}}
                <div class="px-6 py-4" style="background: linear-gradient(135deg, #0B4EA2 0%, #0a3d80 100%);">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            {{-- NRAPA Logo --}}
                            <div class="w-12 h-12 bg-white rounded-xl flex items-center justify-center overflow-hidden shadow-md">
                            ...
                            <div>
                                ...
                                <p class="text-xs" style="color: rgba(255,255,255,0.85);">Member Card</p>
                            </div>
                        </div>
                        {{-- Status Badge --}}
                        @if($this->isExpired)
                        ...
                        @else
                        <span class="px-3 py-1 text-xs font-bold rounded-full uppercase" style="background: #F58220; color: #fff;">Active</span>
                        @endif
                    </div>
                </div>

                {{-- Orange accent stripe --}}
                <div style="height: 3px; background: linear-gradient(90deg, #F58220, #f9a825, #F58220);"></div>
                ...
                <div class="px-6 py-6 space-y-6 bg-white text-zinc-900 dark:bg-white dark:text-zinc-900">
                    ...
                    <div class="space-y-4">
                        <div>
                            <p class="text-xs uppercase tracking-wider" style="color: #0B4EA2;">Member Name</p>
                            <p class="text-zinc-900 dark:text-zinc-900 text-xl font-semibold">{{ $memberName ?: '—' }}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <p class="text-xs uppercase tracking-wider" style="color: #0B4EA2;">Membership No.</p>
                                <p class="text-zinc-900 dark:text-zinc-900 font-mono text-sm font-medium">{{ $membershipNo ?: '—' }}</p>
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-wider" style="color: #0B4EA2;">Type</p>
                                <p class="text-zinc-900 dark:text-zinc-900 text-sm font-medium">{{ $membershipTypeName }}</p>
                            </div>
                        </div>
                        <div>
                            <p class="text-xs uppercase tracking-wider" style="color: #0B4EA2;">Valid Until</p>
                            @if($this->isLifetime)
                            <p class="font-semibold" style="color: #0B4EA2;">Lifetime</p>
                            @else
                    ...
                    <div class="flex flex-col items-center">
                        <div class="p-3 rounded-2xl shadow-lg" style="background: #f8fafc; border: 1px solid rgba(11,78,162,0.15);">
                            <img src="{{ $this->qrCodeUrl }}" alt="Verification QR Code" class="w-48 h-48" loading="lazy">
                        </div>
                        <p class="text-xs mt-3 text-center" style="color: #6B7280;">Scan to verify membership</p>
                    </div>
                </div>

                {{-- Card Footer — NRAPA Blue --}}
                <div class="px-6 py-3" style="background: #0B4EA2;">
                    <p class="text-xs text-center font-medium" style="color: rgba(255,255,255,0.9);">
        ...
        <div class="text-center px-6 max-w-sm">
            <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style="background: rgba(245,130,32,0.2);">
                <svg class="w-8 h-8" style="color: #F58220;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('dashboard') }}" wire:navigate class="inline-flex items-center gap-2 px-4 py-2 text-white rounded-lg" style="background: #0B4EA2;">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                </svg>
                Back to Dashboard
        ...
        <div class="text-center px-6 max-w-sm">
            <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style="background: rgba(11,78,162,0.2);">
                <svg class="w-8 h-8" style="color: #F58220;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('membership.index') }}" wire:navigate class="inline-flex items-center gap-2 px-4 py-2 text-white rounded-lg" style="background: #0B4EA2;">
