...
new #[Layout('layouts.app.sidebar')] #[Title('Review Endorsement Request - Admin')] class extends Component {
    ...
    /**
     * Re-render the endorsement letter PDF for an already-issued request.
     *
     * The PDF is cached on object storage at documents/endorsement-letter-{uuid}.pdf
     * and only regenerated by the download routes when the file is missing.
     * After admins edit firearm details (e.g. correcting/removing serial numbers
     * via the firearm edit modal), the cached PDF retains the old data — this
     * method forces a fresh render that overwrites the cached file at the same
     * path, so subsequent downloads serve the corrected letter.
     *
     * The letter_reference and letter_file_path are intentionally preserved so
     * external references (emails sent to the member, audit history) remain
     * valid.
     */
    public function regenerateLetter(): void
    {
        if (!$this->request->isIssued()) {
            session()->flash('error', 'Letter can only be regenerated for issued endorsements.');
            return;
        }
        ...
        try {
            ...
            // Path is deterministic from the request UUID, but persist it
            // anyway so any record that previously had a NULL path is fixed.
            $this->request->update(['letter_file_path' => $letterPath]);

            AuditLog::create([
                'user_id' => auth()->id(),
                'event' => 'endorsement_letter_regenerated',
                'auditable_type' => EndorsementRequest::class,
                'auditable_id' => $this->request->id,
                'old_values' => null,
                'new_values' => [
                    ...
                    'reason' => 'Admin manually regenerated the endorsement letter PDF.',
                ],
                'ip_address' => request()->ip(),
                'user_agent' => request()->userAgent(),
            ]);
            ...
            $this->request->refresh();
            session()->flash('success', 'Endorsement letter regenerated. The next download will serve the updated PDF.');
        } catch (\Throwable $e) {
            Log::error('Endorsement letter regeneration failed', [
                'request_id' => $this->request->id,
                'error' => $e->getMessage(),
            ]);
            session()->flash('error', 'Failed to regenerate letter: ' . $e->getMessage());
        }
    }
...
<div>
    <x-slot name="header">
        <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Endorsement Review</h1>
                    ...
                    @if($request->isIssued())
                        <div class="text-center py-4">
                            ...
                            @if($request->isIssued())
                                ...
                                {{-- Regenerate the cached PDF — needed after admins edit firearm --}}
                                {{-- details (e.g. correcting serial numbers) on an already-issued letter. --}}
                                <button type="button"
                                    wire:click="regenerateLetter"
                                    wire:confirm="Regenerate the endorsement letter PDF using the current firearm details? The letter reference will not change."
                                    wire:loading.attr="disabled"
                                    class="w-full mt-2 px-4 py-2 border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors flex items-center justify-center gap-2 text-sm font-medium disabled:opacity-50">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                    </svg>
                                    <span wire:loading.remove wire:target="regenerateLetter">Regenerate Letter PDF</span>
                                    <span wire:loading wire:target="regenerateLetter">Regenerating…</span>
                                </button>
