...
<div>
    <x-slot name="header">
        <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Owner Dashboard</h1>
        <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Platform management and administrative oversight</p>
    </x-slot>
    ...
    {{-- Storage Status --}}
    <div class="mb-8 bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Storage Status</h2>
            <a href="{{ route('owner.settings.storage') }}" wire:navigate class="text-sm text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 transition-colors">Configure</a>
        </div>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {{-- Private Bucket --}}
            <div class="p-4 rounded-lg {{ $privateStorageConfigured ? 'bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800' }}">
                <div class="flex items-center gap-3">
                    ...
                    @endif
                    <div>
                        <p class="font-medium {{ $privateStorageConfigured ? 'text-emerald-800 dark:text-emerald-300' : 'text-red-800 dark:text-red-200' }}">Private Bucket (Documents)</p>
                        <p class="text-sm {{ $privateStorageConfigured ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
                            @if($privateStorageConfigured)
                                {{ $privateBucket }}
                            @else
                                Not configured
    ...
    <div class="mb-8 bg-gradient-to-r from-emerald-50 to-blue-50 dark:from-emerald-900/20 dark:to-blue-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800 p-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('owner.settings.index') }}" wire:navigate
                    class="px-6 py-3 bg-nrapa-blue hover:bg-nrapa-blue-dark text-white font-medium rounded-lg transition-colors flex items-center gap-2">
