...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .load-name { font-size: 11px; font-weight: bold; color: #0B4EA2; margin-bottom: 0.5mm; }
        .calibre { font-size: 10px; font-weight: bold; color: #F58220; margin-bottom: 0.5mm; }

        .detail-row { margin-bottom: 0.3mm; line-height: 1.4; }
        ...
        /* Single large label */
        .single-load-name { font-size: 16px; font-weight: bold; color: #0B4EA2; margin-bottom: 2mm; }
        .single-calibre { font-size: 14px; font-weight: bold; color: #F58220; margin-bottom: 3mm; }
        .single-detail { font-size: 11px; margin-bottom: 1.5mm; line-height: 1.4; }
        .single-detail strong { color: #555; }
        .single-footer { font-size: 8px; color: #999; text-align: center; margin-top: 4mm; padding-top: 3mm; border-top: 1px solid #eee; }
    </style>
</head>
<body>
@if($label_layout === '2x7')
                                ...
                                <div class="label-cell">
                                    @if($labelIdx < $label_count)
                                        <div class="label">
                                            ...
                                            @endif
                                            <div class="date-text" style="margin-top: 0.5mm;">{{ now()->format('d M Y') }}</div>
        ...
        <div class="page" @if($i < $label_count - 1) style="page-break-after: always;" @endif>
            <div class="label-single">
                ...
                @if($load->muzzle_velocity)
                    <div class="single-detail"><strong>Vel:</strong> {{ $load->muzzle_velocity }}fps @if($load->velocity_sd) &nbsp; <strong>SD:</strong> {{ $load->velocity_sd }} @endif @if($load->velocity_es) &nbsp; <strong>ES:</strong> {{ $load->velocity_es }} @endif</div>
                @endif
                @if($load->group_size)
                ...
                {{-- Date --}}
                <div style="font-size: 9px; color: #777; margin-top: 2mm;">{{ now()->format('d M Y') }}</div>
